...
export default Vue.extend({
  ...
  beforeMount() {
    const data = this.$store.getters.eventById(this.$route.params.id);
    this.form.id = data.id;
    this.form.title = data.event.title;
    this.form.eventType = data.event.budget
      ? "Праздничные дни"
      : data.event.address
      ? "Мероприятия"
      : "Пометки / Другое";
    this.form.text = data.event.text ? data.event.text : "";
    this.form.budget = data.event.budget ? data.event.budget : "";
    this.form.address = data.event.address ? data.event.address : "";
    this.form.time = data.event.time ? data.event.time : "";
  },
  methods: {
    clearForm() {
      (
